...
{   
	...
	// Variables privadas
	private readonly NotificationsModelCollection _notifications = new();
	private readonly NotificationsModelCollection _buffer = new();
	...
	/// </summary>
	public NotificationModel AddNotification(NotificationModel.NotificationType type, string title, string message, string? urlImage = null)
		...
		if (e.NewSize.Height == 0.0)
		{
			Grid? grdNotifications = sender as Grid;
				...
				if (grdNotifications != null && grdNotifications.Tag != null)
					RemoveNotification(grdNotifications.Tag.ToString() ?? string.Empty);
		}
	}
}
